...
namespace OiLab\OiLaravelInsee\Console\Commands;

use Illuminate\Console\Command;

class InstallAiSkillCommand extends Command
{
    protected $signature = 'oi-insee:install-ai-skill';

    protected $description = '[Deprecated] Use `oi:skills` instead. Install AI assistant skill files for oi-laravel-insee';

    private const SKILL_NAME = 'oilab-laravel-insee';

    private const SECTION = 'oi-lab/oi-laravel-insee rules';
    ...
    private function addSkillToClaudeMd(): void
    {
        ...
        if (! file_exists($claudeMdPath)) {
            ...
            $this->info('Created CLAUDE.md with oi-laravel-insee rules.');
        ...
        if (! str_contains($content, $sectionHeader)) {
            ...
            $this->info('Added oi-laravel-insee rules section to CLAUDE.md.');
        ...
        $this->info('Updated oi-laravel-insee rules section in CLAUDE.md.');
    }
}
